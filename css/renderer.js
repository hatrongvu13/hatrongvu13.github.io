...
(() => {
  ...
  function renderHero() {
    ...
    const cvButton = byId("download-cv-btn");
    const cvPath =
      settings.cvDownload ||
      settings.cvFile ||
      "./assets/cv/Vu-Ha-Trong-CV.pdf";

    if (cvButton && isSafeAssetPath(cvPath)) {
      cvButton.href = cvPath;
      cvButton.hidden = false;
      cvButton.download = settings.pdfFileName || "Vu-Ha-Trong-CV.pdf";
    } else if (cvButton) {
  ...
  function renderProjects() {
    ...
    state.data.projects.forEach((project) => {
      const card = create("article", { className: "project-card gsap-card" });
      ...
      appendTextElement(card, "h3", localized(project.name));

      const type = asText(localized(project.type));
      if (type) appendTextElement(card, "small", type);

      appendTextElement(card, "p", localized(project.description));

      const technologies = asArray(project.tech || project.technologies);
      if (technologies.length) {
        const chips = create("div", { className: "creator-topics" });
      ...
      const sourceLink = createExternalLink(
        project.source,
        translate("sourceCode", "Mã nguồn")
      );
      const demoLink = createExternalLink(
        project.demo,
        translate("liveDemo", "Demo")
      );
      if (sourceLink) actions.appendChild(sourceLink);
      if (demoLink) actions.appendChild(demoLink);
